...
const QString BaseURL = "http://HWW.io/HWWusd.php";
const QString BaseURL2 = "http://HWW.io/HWWbtc.php";
double HWWx;
double HWWbtcx;
...
if (what == BaseURL) // HWW Price
{

    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString HWW = finished->readAll();
    HWWx = (HWW.toDouble());
    HWW = QString::number(HWWx, 'f', 2);

	dollarg = HWW;
}
if (what == BaseURL2) // HWW BTC Price
{

    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString HWWbtc = finished->readAll();
    HWWbtcx = (HWWbtc.toDouble());
    HWWbtc = QString::number(HWWbtcx, 'f', 8);
