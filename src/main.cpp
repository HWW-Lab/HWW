...
const string strMessageMagic = "HWW Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
		nSubsidy = 100000 * COIN;  // 1% Premine
	else if (pindexBest->nHeight <= FAIR_LAUNCH_BLOCK) // Block 210, Instamine prevention
        nSubsidy = 1 * COIN/2;
	else if (pindexBest->nHeight <= 1000000) // Block 1m ~ 3m HWW (33% will go to hybrid masternodes)
		nSubsidy = 2 * COIN;
	else if (pindexBest->nHeight <= 2000000) // Block 2m ~ 4m HWW
		nSubsidy = 3 * COIN;
	else if (pindexBest->nHeight <= 3000000) // Block 3m ~ 3m HWW
		nSubsidy = 1 * COIN;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    //PoS Fixed on Block 1000 v2.0+ HWW
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    // ----------- masternode payments -----------
    // Once upon a time, People were really interested in HWW.
    // So much so, People wanted to bring HWW to the moon. Even Mars, Sooner than the roadster...
    ...
    // People wanted masternodes... Really Bad. But King Carsen was already busy with the rest of HWW
                        ...
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            ...
                            BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                            {
                                ...
                                if (vtx[1].vout[i].scriptPubKey == pubScript)
                                {
                                    ...
                                    if (fDebug) printf("Masternode PoS payee found at block %d: %s who got paid %s HWW (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[1].vout[i].nValue / COIN).c_str(), paidAge, mn.nBlockLastPaid);
                    ...
                    for (unsigned int i = 0; i < vtx[0].vout.size(); i++) {
                        ...
                        BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                        {
                            ...
                            if (vtx[0].vout[i].scriptPubKey == pubScript)
                            {
                                int lastPaid = mn.nBlockLastPaid;
                                int paidAge = pindex->nHeight+1 - lastPaid;
                                if (fDebug) printf("Masternode PoW payee found at block %d: %s who got paid %s HWW (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[0].vout[i].nValue).c_str(), paidAge, mn.nBlockLastPaid);
                                if (paidAge < 150) // TODO: Probably make this check the MN is in the top 50?
                                {
                                    if (fDebug) printf("WARNING: This masternode payment is too aggressive and will not be accepted in v3+\n");
                                }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // HWW: ask for pending sync-checkpoint if any
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "HWW", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "https://www.criptonoticias.com/mercados/10-anos-dolar-devaluado-9999-frente-bitcoin/";
        CTransaction txNew;
        txNew.nTime = 1613622853;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nTime    = 1613622853;
        block.nVersion = 1;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
		    block.nNonce   = 2677138;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xacb3f726f22e363b8710d84d12a64ce5667b765d204c32c7eff4d733a9dc6c3e"));
        block.print();
