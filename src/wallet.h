...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTMN = 4 // ONLY_NONDENOMINATED and not 5000 HWW at the same time
};
...
public:
    ...
    bool SendHWWToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToHWW(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nHWW, int64_t& nAnon, const isminefilter& filter) const
    {
        nHWW = 0;
        nAnon = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable int64_t nCredHWWCached;
    ...
    bool GetCredit(int64_t& nCredHWW, int64_t& nCredAnon, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredHWW = nCredAnon = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredHWWCached = 0;
            nCredAnonCached = 0;
            pwallet->GetCredit(*this, nCredHWWCached, nCredAnonCached, ISMINE_SPENDABLE);
            fCreditSplitCached = true;
        };

        nCredHWW = nCredHWWCached;
